...
static int input_read_next_event(struct input_event *ev, struct libevdev *dev)
{
	...
	if (rc < 0) {
		ERROR_LOG(
			"input_read_next_event: error attempting to get next event. Errcode %d",
			rc);
		return -1;
	}
	return 1;
...
static int input_push_new_pen_position(input_state_t *input_state)
{
	...
	TRACE_LOG(
		"input_get_next_pen_event: Updating touch position: x=%d, y=%d",
		input_state->pen_state.position_x,
		input_state->pen_state.position_y);
...
static int input_push_new_touch_position(input_state_t *input_state)
{
	...
	TRACE_LOG(
		"input_push_new_touch_position: Updating touch position: x=%d, y=%d",
		single_state->position_x,
		single_state->position_y);
	ring_buf_write(&input_state->events_buf, &position_event);
	return 1;
}

static int input_get_next_gpio_event(input_state_t *input_state)
{
	...
	case EV_KEY:
		...
		case KEY_LEFT:;
			...
			TRACE_LOG(
				"input_get_next_gpio_event: Sent gpio pgup event");
			break;
		case KEY_HOME:;
			...
			TRACE_LOG(
				"input_get_next_gpio_event: Sent gpio home event");
			break;
		case KEY_RIGHT:;
			...
			TRACE_LOG(
				"input_get_next_gpio_event: Sent gpio pgdown event");
			break;
		default:
			ERROR_LOG(
				"input_get_next_multitouch_event: Unhandled ev.code %s for KEY event",
	...
	default:
		ERROR_LOG(
			"input_get_next_gpio_event: received unexpected event type %s",
...
static int input_get_next_pen_event(input_state_t *input_state)
{
	struct input_event ev;
	...
	case EV_ABS:
		...
		default:
			ERROR_LOG(
				"input_get_next_pen_event: Unhandled ev.code %s for ABS event",
				libevdev_event_code_get_name(ev.type, ev.code));
			return -1;
		}
		break;
	case EV_KEY:
		...
		default:
			ERROR_LOG(
				"input_get_next_pen_event: Unhandled ev.code %s for KEY event",
				libevdev_event_code_get_name(ev.type, ev.code));
			return -1;
		}
		break;
	case EV_SYN:
		switch (ev.code) {
		case SYN_REPORT:
			// If the touch state of the pen has changed, push the
			// updates immediately to buffer Otherwise, don't send
			// new x/y coordinates here, we only do that if asked
			// nicely for it

			// On touch up, first transmit key up and then new
			// position
			if (input_state->pen_state.touch_state_changed &&
			    !input_state->pen_state.touched) {
			...
			// On touch down, first transmit new position and then
			// key down
			if (input_state->pen_state.touch_state_changed) {
				input_push_new_pen_position(input_state);
		...
		case SYN_DROPPED:
			...
			DEBUG_LOG(
				"input_get_next_pen_event: Events were dropped! State is now probably invalid.");
			return -1;
		default:
			ERROR_LOG(
				"input_get_next_pen_event: Unhandled event code %s for SYN event",
				libevdev_event_code_get_name(ev.type, ev.code));
			return -1;
		}
		break;
	default:
		ERROR_LOG(
			"input_get_next_pen_event: received unexpected event type %s",
			libevdev_event_type_get_name(ev.type));
		return -1;
	}
	return 1;
...
static int input_get_next_multitouch_event(input_state_t *input_state)
{
	struct input_event ev;
	...
	case EV_ABS:
		...
		case ABS_MT_TOUCH_MAJOR:
			input_state->multitouch_state
				.slots[input_state->multitouch_state
					       .current_slot]
				.touch_major = ev.value;
			break;
		case ABS_MT_TOUCH_MINOR:
			input_state->multitouch_state
				.slots[input_state->multitouch_state
					       .current_slot]
				.touch_minor = ev.value;
			break;
		case ABS_MT_WIDTH_MAJOR:
			input_state->multitouch_state
				.slots[input_state->multitouch_state
					       .current_slot]
				.width_major = ev.value;
			break;
		case ABS_MT_WIDTH_MINOR:
			input_state->multitouch_state
				.slots[input_state->multitouch_state
					       .current_slot]
				.width_minor = ev.value;
			break;
		...
		default:
			ERROR_LOG(
				"input_get_next_multitouch_event: Unhandled ev.code %s for ABS event",
				libevdev_event_code_get_name(ev.type, ev.code));
			return -1;
		}
		break;
	case EV_SYN:
		...
		case SYN_REPORT:
			...
			// If the touch state of the first pressed slot has
			// changed, push the updates immediately to buffer
			// Otherwise, don't send new x/y coordinates here, we
			// only do that if asked nicely for it
			...
			if (current_slot->tracking_id_changed &&
				...
				ring_buf_write(&input_state->events_buf,
					       &up_event);
				TRACE_LOG(
					"input_get_next_multitouch_event: Sent mouse up event (from touch)");
			...
			if (current_slot->tracking_id_changed) {
				...
				ring_buf_write(&input_state->events_buf,
					       &down_event);
				TRACE_LOG(
					"input_get_next_multitouch_event: Sent mouse down event (from touch)");
			}
		...
		case SYN_DROPPED:
			// TODO recover from dropped event
			DEBUG_LOG(
				"input_get_next_multitouch_event: Events were dropped! State is now probably invalid.");
			return -1;
		default:
			ERROR_LOG(
				"input_get_next_multitouch_event: Unhandled ev.code %s for SYN event",
				libevdev_event_code_get_name(ev.type, ev.code));
			return -1;
		}
		break;
	default:
		ERROR_LOG(
			"input_get_next_multitouch_event: received unexpected event type %s",
			libevdev_event_type_get_name(ev.type));
		return -1;
	}
	return 1;
...
static void *input_async_handler(void *context)
{
	...
	while (input_state->poll_active) {
		...
		if (input_state->events_requested) {
			if (input_push_new_touch_position(input_state) |
			    input_push_new_pen_position(input_state)) {
...
static int input_identify_input_devices(input_state_t *input_state)
{
	...
	int dev_name_fd = open("/sys/devices/soc0/machine",
			       O_RDONLY | O_NONBLOCK);
	if (dev_name_fd < 0) {
		ERROR_LOG(
			"input_identify_input_devices: Can't open machine name file");
		return -1;
	}
	...
	if (strncmp(RM1_MACHINE_NAME_1,
		    machine_name,
		    strlen(RM1_MACHINE_NAME_1)) == 0 ||
	    strncmp(RM1_MACHINE_NAME_2,
		    machine_name,
		    strlen(RM1_MACHINE_NAME_2)) == 0) {
		DEBUG_LOG(
			"input_identify_input_devices: Device identified as RM1");
		input_state->model = RM1;
	} else if (strncmp(RM2_MACHINE_NAME,
			   machine_name,
			   strlen(RM2_MACHINE_NAME)) == 0) {
		DEBUG_LOG(
			"input_identify_input_devices: Device identified as RM2");
		input_state->model = RM2;
	} else {
		ERROR_LOG(
			"input_identify_input_devices: Could not identify device type (machine name: %s)",
			machine_name);
		return -1;
	}
	...
	if (dev_dir == NULL) {
		ERROR_LOG(
			"input_identify_input_devices: Can't open device directory.");
		return -1;
	}
	...
	while ((entry = readdir(dev_dir)) != NULL) {
		if (strncmp("event", entry->d_name, strlen("event")) == 0) {
			...
			if (fd < 0) {
				ERROR_LOG(
					"input_identify_input_devices: Could not open device file %s (errno %d)",
					filename,
					errno);
				return -1;
			}
			...
			if (libevdev_new_from_fd(fd, &dev) < 0) {
				ERROR_LOG(
					"input_identify_input_devices: Could not create device from fd for path %s",
					filename);
				return -1;
			}
			if (libevdev_has_event_code(dev, EV_ABS, ABS_MT_SLOT)) {
				DEBUG_LOG(
					"input_identify_input_devices: Identified %s as multitouch input",
					filename);
				input_state->touch_dev = dev;
			}
			if (libevdev_has_event_code(dev, EV_KEY, KEY_POWER)) {
				DEBUG_LOG(
					"input_identify_input_devices: Identified %s as GPIO input",
					filename);
				input_state->gpio_dev = dev;
			}
			if (libevdev_has_event_code(dev, EV_KEY, BTN_STYLUS)) {
				DEBUG_LOG(
					"input_identify_input_devices: Identified %s as pen input",
					filename);
				input_state->pen_dev = dev;
			}
		}
	}
	if (input_state->pen_dev == NULL || input_state->touch_dev == NULL ||
	    input_state->gpio_dev == NULL) {
		ERROR_LOG(
			"input_identify_input_devices: Could not identify all required devices.");
		return -1;
	}
...
int input_initialize(input_state_t *input_state, nsfb_t *nsfb)
{
	if (input_identify_input_devices(input_state) != 0) {
		ERROR_LOG(
			"input_initialize: Could not determine input device locations.");
		return -1;
	}
	...
	if (num_slots == -1) {
		ERROR_LOG(
			"input_initialize: Can't determine number of slots for multitouch device");
		return -1;
	}
	input_state->multitouch_state.slots = malloc(
		sizeof(input_single_state_t) * num_slots);

	const struct input_absinfo *absinfo_x = libevdev_get_abs_info(
		input_state->touch_dev, ABS_MT_POSITION_X);
	const struct input_absinfo *absinfo_y = libevdev_get_abs_info(
		input_state->touch_dev, ABS_MT_POSITION_Y);
	...
	DEBUG_LOG(
		...
		num_slots,
		input_state->multitouch_state.min_x,
		input_state->multitouch_state.max_x,
		input_state->multitouch_state.min_y,
		input_state->multitouch_state.max_y);

	const struct input_absinfo *pen_absinfo_x = libevdev_get_abs_info(
		input_state->pen_dev, ABS_X);
	const struct input_absinfo *pen_absinfo_y = libevdev_get_abs_info(
		input_state->pen_dev, ABS_Y);
	...
	DEBUG_LOG(
		"input_initialize: pen device x is between %d and %d, y is between %d and %d)",
		input_state->pen_state.min_x,
		input_state->pen_state.max_x,
		input_state->pen_state.min_y,
		input_state->pen_state.max_y);
	...
	int thread_create_result = pthread_create(
		&thread, NULL, input_async_handler, input_state);
#ifdef _GNU_SOURCE
	pthread_setname_np(thread, "input");
#endif
	if (thread_create_result != 0) {
		ERROR_LOG(
			"input_initialize: could not initialize async poll thread");
		return -1;
	}
...
int input_finalize(input_state_t *input_state)
{
	...
	DEBUG_LOG(
		"input_finalize: All evdev devices and the event buffer were closed");
